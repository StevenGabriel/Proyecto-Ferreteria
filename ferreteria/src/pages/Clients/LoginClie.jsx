...
function LoginClie() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      setLoading(true);
      const res = await loginUser({ correo, contrasena, rol: 'Cliente' });

      // Guardar sesión en LocalStorage
      if (res && res.user) {
        localStorage.setItem("cyc_client_session", JSON.stringify(res.user));
      }

      // Redirigir al catálogo de la tienda
      navigate("/");
    } catch (err) {
  ...
  return (
    <div className="min-h-screen w-full bg-slate-950 text-white flex items-center justify-center p-4 relative overflow-hidden font-sans selection:bg-cyan-500 selection:text-slate-950">
      ...
      <div className="absolute top-1/3 -right-32 w-96 h-96 bg-cyan-500/10 rounded-full blur-3xl pointer-events-none"></div>
      <div className="absolute bottom-1/3 -left-32 w-96 h-96 bg-blue-600/10 rounded-full blur-3xl pointer-events-none"></div>
      <div className="absolute top-0 inset-x-0 h-px bg-gradient-to-r from-transparent via-cyan-500/30 to-transparent"></div>

      {/* Tarjeta de Inicio de Sesión de Cliente */}
      <div className="w-full max-w-md bg-slate-900/80 backdrop-blur-xl border border-slate-800/90 rounded-3xl p-8 sm:p-10 shadow-2xl relative z-10 space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-slate-400 font-medium">
            Inicia sesión para gestionar tus pedidos y cotizaciones <br />
        ...
        {/* Registro y Enlaces */}
        <div className="pt-2 border-t border-slate-800/80 space-y-3 text-center text-xs text-slate-400">
          <div>
