...
function LoginAdm() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      setLoading(true);
      const res = await loginUser({ correo, contrasena, rol: 'Administrador' });
      ...
      const msg = err.response?.data?.message || "Credenciales incorrectas o servidor no disponible.";
      setErrorMessage(msg);
    } finally {
      setLoading(false);
    }
  ...
  const handleFillDemo = () => {
    setCorreo("admin@ferreteria.com");
  ...
  return (
    <div className="min-h-screen w-full bg-slate-950 text-white flex items-center justify-center p-4 relative overflow-hidden font-sans selection:bg-cyan-500 selection:text-slate-950">
      {/* Efectos de fondo con resplandor ambiental */}
      ...
      <div className="w-full max-w-md bg-slate-900/80 backdrop-blur-xl border border-slate-800/90 rounded-3xl p-8 sm:p-10 shadow-2xl relative z-10 space-y-6">
        ...
        <div className="text-center space-y-2">
          <Link to="/" className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-gradient-to-tr from-cyan-500 to-blue-600 text-white font-black text-2xl shadow-lg shadow-cyan-500/25 hover:scale-105 transition-transform mb-2">
            C
          </Link>
          ...
          <p className="text-xs text-slate-400 font-medium">
            Panel de control, inventario y ventas <br />
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between items-center">
              ...
              <button
                ...
                onClick={() => alert("Comunícate con el administrador principal para restablecer tu clave.")}
              >
        ...
        <div className="pt-2 border-t border-slate-800/80 text-center">
          <button
            ...
            className="px-3 py-1.5 bg-slate-950/80 hover:bg-slate-800 text-slate-400 hover:text-cyan-400 border border-slate-800 rounded-xl text-[11px] font-bold transition-all inline-flex items-center gap-1.5"
          >
        ...
        <div className="text-center pt-2 text-xs text-slate-400">
          ¿Eres un cliente buscando comprar?{" "}
          <Link to="/" className="text-cyan-400 hover:text-cyan-300 font-bold ml-1 transition-colors">
            Ir a la Tienda / Catálogo →
          </Link>
        </div>
      </div>
    </div>
