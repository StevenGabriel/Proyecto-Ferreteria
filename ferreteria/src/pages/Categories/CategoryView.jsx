...
function CategoryView() {
  ...
  const [activeMenu, setActiveMenu] = useState("productos-categorias");

  // Estados para el Modal de Confirmación de Eliminación
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
  const [categoryToDelete, setCategoryToDelete] = useState(null);

  // Estados para el Modal de Agregar/Editar Categoría (Upsert)
  ...
  const confirmDelete = () => {
    if (categoryToDelete) {
      deleteCategory(categoryToDelete.CategoriaID)
        .then(() => {
          setCategories(categories.filter((c) => c.CategoriaID !== categoryToDelete.CategoriaID));
  ...
  const openUpsertModal = (mode, category = null) => {
    setUpsertMode(mode);
    setFormError("");
    if (mode === "edit" && category) {
      setCategoryId(category.CategoriaID);
    ...
    if (upsertMode === "create") {
      createCategory(categoryData)
        .then((newCategory) => {
          ...
          setToastMessage("¡Categoría registrada con éxito!");
          setShowToast(true);
          playSuccessSound();
        ...
        .catch((error) => {
          ...
          setFormError("Error al registrar la categoría.");
        });
    } else {
      updateCategory(categoryId, categoryData)
        .then((updated) => {
          setCategories(categories.map((c) => (c.CategoriaID === categoryId ? updated : c)));
          setToastMessage("¡Categoría actualizada con éxito!");
          setShowToast(true);
          playSuccessSound();
          setIsUpsertModalOpen(false);
        })
  ...
  // Filtrar categorías por término de búsqueda (nombre)
  const filteredCategories = categories.filter((category) => {
    const nombreVal = category.Nombre ? category.Nombre.toLowerCase() : "";
  ...
  return (
    <div className="bg-slate-950 text-white min-h-screen font-sans flex">
        ...
        <div className="p-6 md:p-8 max-w-[1920px] w-full mx-auto space-y-6 flex-1">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <p className="text-slate-400 text-sm mt-1">Administra las categorías de productos (ej. herramientas, eléctricos, etc.) de la ferretería.</p>
            </div>
            <button
          ...
          {/* Tabla de Categorías Estilizada en Cristal */}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left text-slate-300">
                <thead className="text-xs uppercase bg-slate-900/60 text-slate-400 border-b border-slate-800">
                  <tr>
                    <th scope="col" className="px-6 py-4 font-bold tracking-wider">Nombre de la Categoría</th>
                    ...
                    currentPageCategories.map((category) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-slate-400 max-w-xs truncate">
                          {category.Descripcion ? category.Descripcion : <span className="text-slate-600 italic text-xs">Sin descripción</span>}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-right text-xs font-semibold space-x-2">
                          <button
                            onClick={() => openUpsertModal("edit", category)}
        ...
        <div className="bg-slate-950/80 backdrop-blur-sm fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-md w-full p-6 space-y-6 shadow-2xl animate-scale-in">
            ...
            <p className="text-sm text-slate-300 leading-relaxed">
              ¿Estás seguro de que deseas eliminar permanentemente la categoría <span className="font-bold text-white">"{categoryToDelete?.Nombre}"</span> del catálogo de la ferretería?
            </p>
      ...
      {/* Modal de Agregar/Editar Categoría (Upsert) */}
      {isUpsertModalOpen && (
        <div className="bg-slate-950/80 backdrop-blur-sm fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="space-y-1">
                <h3 className="text-lg font-bold text-white">
                  {upsertMode === "create" ? "Registrar Nueva Categoría" : "Editar Categoría"}
                </h3>
                <p className="text-slate-400 text-xs mt-0.5">Completa los datos de la categoría a continuación.</p>
              </div>
            </div>
            ...
            <form onSubmit={handleUpsertSubmit} className="space-y-5">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400 block">
                  Nombre de la Categoría <span className="text-cyan-400">*</span>
                </label>
                <input
                  type="text"
                  placeholder="Ej. Herramientas, Eléctricos, Accesorios de seguridad..."
                  value={nombre}
              ...
              <div className="space-y-2">
                ...
                <textarea
                  placeholder="Escribe detalles adicionales sobre esta categoría..."
                  value={descripcion}
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                >
                  {upsertMode === "create" ? "Registrar Categoría" : "Guardar Cambios"}
                </button>
              </div>
            </form>
          </div>
